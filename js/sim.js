...
(function (global) {
        ...
        forNeighbors(sim, u.x, u.y, 2, (o) => {
          if (o !== u && o.race === u.race && o.age > o.adultAt && u.breedCd <= 0) {
            ...
            if (R.likes.indexOf(b) >= 0 || sim.rng() < 0.3) {
              // At most one birth per step, at the chance the elapsed time
              // warrants. At Real Time that is a vanishing fraction and the
              // pacing is the old one; at a year a second it is a certainty
              // whenever a pair is ready, and the species caps above are what
              // stop that from becoming a plague of rabbits.
              if (sim.rng() < Math.min(1, dt / interval)) {
                spawnUnit(sim, u.race, u.x + sim.rng() - 0.5, u.y + sim.rng() - 0.5);
                u.breedCd = interval; u.food -= 0.3;
              }
            }
          }
        });
  ...
  function updateVillage(sim, v, dt) {
    ...
    if (v.pop < v.cap && v.food > 8 + v.pop * 0.6 && sim.units.length < sim.UNIT_CAP) {
      ...
      // A birth is the archetypal hazard: a chance per unit time, saturating
      // at one. Multiplying the old per-tick chance by dt would make every
      // settlement give birth on every step the moment the dial moved, and
      // the world would be at the population cap within a second of leaving
      // Real Time.
      if (sim.rng() < hazard(0.22 * R.breed * (0.3 + room), dt)) {
  ...
  global.PD.Sim = {
    ...
    hazard, hazardFast, rateOver, approach, now, years, subsolar, seasonAt, isNightAt,
    TICK_SLOW, TICK_FAST,
